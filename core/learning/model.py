...
class Model:
    ...
    def train(self, X: np.ndarray, use_cache: bool = True) -> None:
        """
    ...
    def visualize(self, X: np.ndarray) -> None:
        ...
        df = pd.DataFrame(X, columns=["timeToSchool", "cl_experience", "school_experience", "priority", "ma_availability", "mobility", "geschlecht_relevant", "qualifications_met"])
        df["anomaly"] = self.predict(X)
        df["score"] = self.score_samples(X)
        
        g = sns.pairplot(df, hue="anomaly", height=2.5, palette={1: 'blue', -1: 'red'}, diag_kind="kde", kind="hist")

        # Bar plot of the number of normal vs anomaly points
        plt.figure(figsize=(8, 6))
        sns.countplot(x="anomaly", data=df, palette={1: 'blue', -1: 'red'}, hue="anomaly", legend=False)
