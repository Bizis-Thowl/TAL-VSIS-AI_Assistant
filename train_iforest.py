...
def main():
    ...
    if use_cache and os.path.exists(full_dataset_path):
        ...
        min_date, max_date = (
            datetime.strptime("2026-03-01", "%Y-%m-%d").date(),
            datetime.strptime("2026-04-28", "%Y-%m-%d").date(),
        )
        ...
        for current_date in daterange(min_date, max_date + timedelta(days=1)):
            ...
            if filtered_vertretungen:
                ...
                ma_assignments = data_processor.get_ma_assignments(
                    rescheduled_ma_records
                )
                assigned_mas = list(ma_assignments.keys())
                assigned_clients = list(ma_assignments.values())

                client_record_assignments = (
                    data_processor.get_client_record_assignments(
                        mabw_records["open_clients"]
                    )
                )

                clients_df, mas_df = data_processor.create_day_dataset(
                    assigned_clients, assigned_mas, current_date
                )
                ...
                mas_df["available_until"] = mas_df["id"].map(
                    lambda x: next(
                        ...
                        for item in rescheduled_ma_records
                        if item["mavertretend"]["id"] == x
                    ),
                    None,
                )
                clients_df["available_until"] = clients_df["id"].map(
                    lambda x: next(
                        (
                            datetime.strptime(item["enddatum"], "%Y-%m-%d")
                            for item in rescheduled_ma_records
                            if item["klientzubegleiten"]["id"] == x
                        ),
                        None,
                    )
                )
                ...
                single_df = create_single_df(
                    clients_df, mas_df, replacements, current_date
                )
    ...
    print(
        f"model.decision_function(X_train[-1]): {model.get_decision_function(test_row)}"
    )
